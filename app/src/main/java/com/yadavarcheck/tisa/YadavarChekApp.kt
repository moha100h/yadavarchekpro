    ...
    private fun createNotificationChannels() {
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            ...
            val reminderChannel = NotificationChannel(
                CHANNEL_REMINDER, "یادآور چک", NotificationManager.IMPORTANCE_HIGH
            ).apply { description = "یادآوری سررسید چک‌ها"; enableVibration(true) }
            val overdueChannel = NotificationChannel(
                CHANNEL_OVERDUE, "چک‌های معوق", NotificationManager.IMPORTANCE_HIGH
            ).apply { description = "هشدار چک‌های معوق"; enableVibration(true) }
